...
csv_file_object = csv.reader(open('input/train.csv', 'rbU'))       # Load in the csv file
...
# First read in test
test_file = open('input/test.csv', 'rbU')
test_file_object = csv.reader(test_file)
header = test_file_object.next()

# Also open the a new file so I can write to it. 
output_file = "output/genderclassmodel.csv"
...
for row in test_file_object:
    ...
    if row[3] == 'female':
        predictions_file_object.writerow([row[0], "%d" % int(survival_table[ 0,\
        float(row[1]) - 1, bin_fare ])])
    else:
        predictions_file_object.writerow([row[0], "%d" % int(survival_table[ 1, \
        float(row[1]) - 1, bin_fare])])
